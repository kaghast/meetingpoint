...
async function startServer() {
  ...
  app.post('/api/admin/sessions', verifyAdminAuth, (req, res) => {
    ...
    if (store.sessions.length === 1 || !store.activeSessionId) {
  ...
  // Delete session: Allowed ONLY if NOT archived!
  app.delete('/api/admin/sessions/:id', verifyAdminAuth, (req, res) => {
    ...
    // STRICT REQUIREMENT: Arşivlenmeyen oturumlar silinebilir olmalı! Arşivlenmiş silinemez!
    if (existing.isArchived) {
      return res.status(400).json({ 
        error: 'Arşivlenmiş oturumlar silinemez. Silmek için önce arşivden çıkarınız.' 
      });
    }
  ...
  app.post('/api/admin/sessions/:id/archive', verifyAdminAuth, (req, res) => {
    ...
    sess.isArchived = Boolean(isArchived);
  ...
  app.post('/api/admin/sessions/:id/activate', verifyAdminAuth, (req, res) => {
    ...
    store.sessionEnded = Boolean(sess.sessionEnded);
    ...
    res.json({ success: true, activeSession: sess });
  });
